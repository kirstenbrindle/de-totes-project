def lambda_handler(event, context):
    ...
    # if boolean is true - get_most_recent_file() - Variables for each - sales_order, design, address, currency, staff, department, counterparty and should be saved as variables.
    # dataframes = read_csv_to_df(Variables for each - sales_order, design, address, currency, staff, department, counterparty)
    ...
    # If boolean is False
    # Use filename from get_key_name - read the file using read_csv_to_df(file_name) - saved as a dataframe.
    # if file_name contains sales_order - invoke make_dim_sales_order() formatter func.
    #  if file_name contains design - invoke make_dim_design() formatter func.
    # Output of dim sales order or dim design passed into write_to_parquet function.
